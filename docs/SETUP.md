...
holds five attempts of the observed cost — but the privileged default is 5
...
evidence` with no score. The greeter and lock screen never pay that arithmetic:
grouped collection gathers the whole vote window inside one transaction, at one
attempt's cost. Check for `camera-tune` reporting `MeasuredSequential` and for
that phrase in `irlume logs`.
...
at ~7.3s on a sequential pair), and a *refused* attempt holds the camera for the
