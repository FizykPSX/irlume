...
- A KDE Plasma System Settings module (KCM): read-only status, diagnostics,
  ...
  the session provides xdg-terminal-exec). Packaging: Fedora ships it as
  the `irlume-kcm` subpackage, Arch as a split package, the PPA as an
  `irlume-kcm` binary package (Ubuntu 26.04), and the Nix flake exposes
  `packages.irlume-kcm` plus a `services.irlume.kcm.enable` module option.
